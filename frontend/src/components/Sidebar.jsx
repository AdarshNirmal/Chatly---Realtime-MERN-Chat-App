...
function SideBar() {
  ...
  return (
    <div className={`lg:w-[30%] w-full h-full overflow-hidden lg:block bg-slate-200 dark:bg-slate-900 relative ${!selectedUser ? "block" : "hidden"} transition-colors duration-300`}>
      ...
      {/* Theme toggle */}
      <div
        className='w-[60px] h-[60px] rounded-full overflow-hidden flex justify-center items-center bg-white dark:bg-slate-800 shadow-gray-500 dark:shadow-black text-gray-700 dark:text-white cursor-pointer shadow-lg fixed bottom-[90px] left-[10px] z-50'
        onClick={toggleTheme}
      >
        {theme === "light"
      ...
      {/* Logout */}
      <div
        className='w-[60px] h-[60px] mt-[10px] rounded-full overflow-hidden flex justify-center items-center bg-[#20c7ff] shadow-gray-500 dark:shadow-black text-white cursor-pointer shadow-lg fixed bottom-[20px] left-[10px] z-50'
      ...
      <div className='w/full h/[300px] bg-[#20c7ff] dark:bg-slate-800 rounded-b-[30%] shadow-gray-400 dark:shadow-black shadow-lg flex flex-col justify-center px/[20px] transition-colors duration-300'>
        ...
        <div className='w/full flex justify-between items.center'>
          <h1 className='text-gray-800 dark:text-gray-100 font-bold text-[25px]'>
            Hii , {userData.name || "user"}
          </h1>
          <div
            className='w/[60px] h/[60px] rounded-full overflow-hidden flex justify-center items.center bg-white dark:bg-gray-700 cursor-pointer shadow-gray-500 dark:shadow-black shadow-lg'
        ...
        <div className='w/full flex items.center gap/[20px] overflow-y-auto py/[18px]'>
          {!search && (
            <div
              className='w/[60px] h/[60px] mt/[10px] rounded-full overflow-hidden flex justify-center items.center bg-white dark:bg-gray-700 shadow-gray-500 dark:shadow-black cursor-pointer shadow-lg min-w/[60px]'
          ...
          {!search && otherUsers?.map((user) => (
            ...
            <div
              key={user._id}
              className='relative rounded-full shadow-gray-500 dark:shadow-black bg-white dark:bg-gray-700 shadow-lg flex justify-center items.center mt/[10px] cursor-pointer min-w/[60px]'
          ...
          {search && (
            <form className='w/full h/[60px] bg-white dark:bg-gray-700 shadow-gray-500 dark:shadow-black shadow-lg flex items.center gap/[10px] mt/[10px] rounded-full overflow-hidden px/[20px] relative'>
              ...
              <input
                ...
                className='w/full h-full p/[10px] text/[17px] outline-none border-0 bg-transparent dark:text-white'
                onChange={(e) => setInput(e.target.value)}
                value={input}
              />
              <RxCross2
                className='w/[25px] h/[25px] cursor-pointer dark:text-white'
      ...
      {/* Main list – sirf users, bina lastMsg/unread */}
      <div className='w/full h/[50%] overflow-auto flex flex-col gap/[20px] items.center mt/[20px] pb/[100px]'>
        {otherUsers?.map((user) => (
          <div
            key={user._id}
            className='w/[93%] h/[56px] flex items.center gap/[20px] shadow-gray-500 dark:shadow-black bg-white dark:bg-gray-800 shadow-lg rounded-full hover:bg-[#78cae5] dark:hover:bg-gray-700 cursor-pointer transition-colors'
            onClick={() => dispatch(setselectedUser(user))}
          >
